import { type Interrupt } from "@/lib/types";import { HITLRequest } from "@/components/thread/agent-inbox/types";
...
export function isAgentInboxInterruptSchema(
  ...
  if (!valueAsObject || typeof valueAsObject !== "object") return false;
  ...
  if (!Array.isArray(actionRequests) || actionRequests.length === 0)
    return false;
  if (!Array.isArray(reviewConfigs) || reviewConfigs.length === 0) return false;

  const hasValidActionRequests = actionRequests.every(
    (request) =>
      request &&
  ...
  const hasValidConfigs = reviewConfigs.every(
    (config) =>
      config &&
